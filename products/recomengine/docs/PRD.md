...
| SDK Bundle Size | < 10KB gzipped | `gzip -c recomengine.v1.js | wc -c` |
...
graph TD
    subgraph Users
        ...
        Admin["Platform Admin<br/>(Marcus)"]
    end
...
**Acceptance Criteria**:
- Given `<script src="recomengine.v1.js" data-api-key="rk_live_..." async></script>` on a page, when the page loads, then the SDK initializes and renders recommendation widgets within 500ms, and the bundle is < 10KB gzipped
- Given a `<div data-recomengine-placement="product-page">` element, when it enters the viewport, then the SDK fetches recommendations and renders product cards, and a `recommendation_impressed` event is auto-tracked
...
- Given a programmatic integration, when `RecomEngine.getRecommendations({ userId, limit })` is called, then a Promise resolving to recommendation data is returned
...
**Acceptance Criteria**:
- Given a tenant's widget configuration, when admin sets `{ layout: "carousel", columns: 4, showPrice: true, ctaText: "Add to Cart", theme: { primaryColor: "#ff6600" } }`, then the SDK renders recommendations using the specified layout and styling
...
sequenceDiagram
    ...
    Site->>SDK: Page load (script tag)
    SDK->>API: GET /recommendations?userId=U1&limit=8
    API->>API: Authenticate API key (HMAC lookup)
    API->>API: Check tenant status (active?)

    alt A/B Test Active
        API->>API: Hash(userId + expId) -> assign variant
    end
    ...
    alt Cache Hit
        ...
        API->>DB: Fetch user events + catalog
        DB-->>API: Events + products
        API->>Engine: Compute recommendations (strategy)
        Engine-->>API: Ranked product list with scores
        API->>Cache: Store with 5-min TTL
    end

    API-->>SDK: 200 { data: [products], meta: {strategy, cached} }
    SDK->>Site: Render widget (grid/carousel/list)
    SDK->>API: POST /events {type: recommendation_impressed}
    Note over Site,SDK: User clicks a product
    SDK->>API: POST /events {type: recommendation_clicked}
...
```mermaid
flowchart TD
    A[User action on merchant site] --> B[SDK / Backend sends event]
...
stateDiagram-v2
    ...
    Running --> Completed: Manually complete / auto-complete
...
erDiagram
    ...
    Tenant {
        ...
        json config "defaultStrategy, corsOrigins, excludePurchased"
        string ownerId FK
        datetime createdAt
        datetime updatedAt
    }
    ...
    ApiKey {
        ...
        string keyPrefix "rk_live_ or rk_test_"
        enum permissions "read | read_write"
        datetime lastUsedAt
        datetime revokedAt
    }
    ...
    CatalogItem {
        string id PK
        string tenantId FK
        string productId "UK with tenantId"
        string name
        string category
        decimal price
        string imageUrl
    ...
    Experiment {
        ...
        int trafficSplit "1-99%"
        enum metric "ctr | conversion_rate | revenue_per_visitor"
    ...
    ExperimentResult {
        ...
        string variant "control | variant"
        int impressions
        int clicks
        int conversions
        decimal revenue
    ...
    WidgetConfig {
        string id PK
        string tenantId FK
        string placementId "UK with tenantId"
        enum layout "grid | carousel | list"
        int columns "2-6"
        json theme "primaryColor, fontFamily"
        int maxItems "4-20"
        boolean showPrice
        string ctaText
    }
    ...
    RevenueAttribution {
        ...
        int attributionWindowMs "default 1800000"
    }
```
...
- **FR-002**: Each tenant MUST have isolated data storage (events, catalog, recommendations, experiments); all queries scoped by tenantId
...
- **FR-026**: Collaborative filtering: recommends products based on similar user behavior (cosine similarity on interaction vectors); requires 1,000+ users with 5+ events
- **FR-027**: Content-based: recommends products with similar catalog attributes (category, description keywords, price range); works with 50+ products
...
| `/api/v1/tenants/:id/widgets` | GET, POST | JWT / API Key | Widget config list and creation |
| `/api/v1/tenants/:id/widgets/:placementId` | GET, PUT, DELETE | JWT / API Key | Widget config CRUD |
...
| Redis failure degrades recommendation latency and rate limiting | High | Low | In-memory fallback for rate limiting; synchronous DB computation on cache miss (still < 100ms for small catalogs) |
...
| SDK errors break merchant site | Critical | Low | SDK wrapped in try/catch; fails silently; no console errors; zero external dependencies; no global namespace pollution beyond `window.RecomEngine` |
| Multi-tenant data leak (tenant A sees tenant B's data) | Critical | Low | All queries scoped by tenantId via Prisma middleware; integration tests verify isolation; security audit before launch |
| A/B test results misleading with small sample sizes | Medium | Medium | Dashboard shows "Low confidence" badge for variants with < 500 users; minimum sample size warning in API response |
| Recommendation quality perception ("why is it showing me this?") | Medium | Medium | Human-readable `reason` field in every recommendation; transparency builds trust |
| API key leaked in client-side code | High | Medium | Read-only keys for SDK (cannot write events or access other tenants); key rotation supported; monitoring for unusual usage patterns |
...
| Placement | A location on a merchant's site where recommendations are displayed (e.g., "product-page", "cart-sidebar") |
