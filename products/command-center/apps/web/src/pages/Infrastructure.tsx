...
export default function Infrastructure() {
  ...
  if (loading) return <div className="animate-pulse"><div className="h-8 bg-slate-800 rounded w-40 mb-6" /></div>;
  if (!data) return <p className="text-rose-400">Failed to load infrastructure</p>;

  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-1">Infrastructure</h1>
      <p className="text-slate-500 mb-8">Ports, CI/CD pipelines, and services</p>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 mb-6">
        ...
        {data.ports.length === 0 ? (
          <p className="text-slate-500 text-sm">No port assignments found in registry</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {data.ports.map((p) => (
              <div key={p.product} className="bg-slate-800/50 rounded-lg p-3 flex items-center justify-between">
                <span className="text-sm text-slate-300 font-medium">{p.product}</span>
                <div className="flex gap-3 text-xs">
                  {p.frontendPort && (
                    <span className="text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded">:{p.frontendPort}</span>
                  )}
                  {p.backendPort && (
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        ...
        {data.pipelines.length === 0 ? (
          <p className="text-slate-500 text-sm">No CI pipelines found</p>
        ) : (
          <div className="space-y-2">
            {data.pipelines.map((p) => (
              <div key={p.filename} className="flex items-center gap-3 bg-slate-800/50 rounded-lg p-3">
                ...
                <div>
                  <p className="text-sm text-slate-300">{p.filename}</p>
                  <p className="text-xs text-slate-600">{p.product}</p>
                </div>
              </div>
            ))}
          </div>
