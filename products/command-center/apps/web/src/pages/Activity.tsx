...
export default function Activity() {
  ...
  if (!data) return <p className="text-rose-400">Failed to load activity</p>;

  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-50 mb-1">Activity Feed</h1>
      <p className="text-slate-500 mb-8">Audit trail and commit history</p>

      <div className="bg-slate-900 border border-slate-700 rounded-xl divide-y divide-slate-700">
        {data.activity.length === 0 && (
          <p className="text-slate-500 text-sm p-6">No activity found</p>
        )}
        {data.activity.map((item, i) => (
          <div key={i} className="p-4 flex items-start gap-4 hover:bg-slate-800/50 transition-colors">
            {/* Type indicator */}
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
              item.type === 'commit' ? 'bg-purple-500/20 text-purple-400' : 'bg-indigo-500/20 text-indigo-400'
            }`}>
              {item.type === 'commit' ? (
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-1 text-xs text-slate-500">
                ...
                {item.data.product && <span className="text-indigo-400">{item.data.product}</span>}
