...
export default function Protocols() {
  ...
  if (!data) return <p className="text-red-400">Failed to load protocols</p>;
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 mb-6">Governing operating protocols for all ConnectSW agents</p>
      ...
      {/* Protocol cards */}
      {filtered.length === 0 && (
        <p className="text-gray-500 text-sm">No protocols in this category</p>
      )}
          ...
          return (
            ...
            >
              <div className="p-5">
                ...
                {/* Source attribution */}
                {protocol.source && (
                  <p className="text-[11px] text-gray-600 italic mb-3 line-clamp-1" title={protocol.source}>
                    {protocol.source}
                  </p>
                )}
                ...
                {protocol.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {protocol.tags.map((tag) => (
                      <span key={tag} className="px-2 py-0.5 rounded-full text-[10px] bg-gray-800 text-gray-500 border border-gray-700">
                        {tag}
                      </span>
                    ))}
                  </div>
              ...
              {/* Expanded content */}
              {isExpanded && (
                <div className="border-t border-gray-800 px-5 py-4 max-h-[600px] overflow-y-auto">
...
function LoadingSkeleton() {
  return (
    <div className="animate-pulse">
      ...
      <div className="h-4 bg-gray-800 rounded w-96 mb-6" />
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-8">
        {[...Array(6)].map((_, i) => <div key={i} className="h-24 bg-gray-800 rounded-xl" />)}
      </div>
      <div className="flex gap-1 mb-6 border-b border-gray-800">
        {[...Array(6)].map((_, i) => <div key={i} className="h-8 w-24 bg-gray-800 rounded" />)}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {[...Array(6)].map((_, i) => <div key={i} className="h-40 bg-gray-800 rounded-xl" />)}
      </div>
    </div>
  );
}
