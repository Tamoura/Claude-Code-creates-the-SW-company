  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:border-slate-700 transition-colors">
      ...
      <div className="p-6">
        <div className="flex items-start justify-between gap-4 mb-3">
          ...
          <div className={`flex-shrink-0 text-right px-4 py-2 rounded-lg border ${scoreBg(report.overallScore)}`}>
            {report.overallScore !== null ? (
              ...
              <p className="text-sm text-slate-500">No score</p>
            )}
          </div>
        </div>
        ...
        {/* Excerpt */}
        {report.excerpt && (
          <p className="text-sm text-slate-400 leading-relaxed line-clamp-3 mb-4">{report.excerpt}</p>
        )}
      ...
      {expanded && (
        <div className="border-t border-slate-800 p-6 bg-slate-950/50 max-h-[60vh] overflow-y-auto">
...
export default function AuditReports() {
  ...
  if (!data) return <p className="text-rose-400">Failed to load audit reports</p>;
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500">Code quality audits across all products</p>
      </div>
      ...
      {reports.length === 0 ? (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
          ...
          <p className="text-slate-400 text-sm">No audit reports found</p>
          <p className="text-slate-600 text-xs mt-1">
            Run <code className="text-indigo-400">/audit [product]</code> to generate a report
          </p>
        </div>
      ) : (
        <div className="space-y-4">
