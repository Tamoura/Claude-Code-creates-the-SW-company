...
export default function DependencyGraph() {
  ...
  if (!data) return <p className="text-rose-400">Failed to load dependency graph</p>;
  ...
  return (
    <div>
      ...
      <p className="text-slate-500 mb-8">Visualize relationships between products, packages, and agents</p>
              ...
              <tbody className="divide-y divide-slate-800">
                {sortedNodes.map((node) => (
                  <tr key={node.id} className="hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-300">{node.label}</td>
                    <td className="px-4 py-3 text-sm text-slate-500">{node.group}</td>
                    <td className="px-4 py-3 text-sm text-slate-400">{countConnections(node.id, edges)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function LoadingSkeleton() {
  return (
    <div className="animate-pulse">
      ...
      <div className="h-4 bg-slate-800 rounded w-80 mb-8" />
      <div className="grid grid-cols-3 gap-4 mb-8">
