...
test.describe('Analytics Flow', () => {
  ...
  test.describe('Analytics Empty State', () => {
    test('shows empty state when no observations exist', async ({ page }) => {
      ...
      // "Log Observation" CTA link — scoped to main content area to avoid
      // matching the sidebar's "Log Observation" link as well
      const logLink = page.getByRole('main').getByRole('link', { name: 'Log Observation' });
