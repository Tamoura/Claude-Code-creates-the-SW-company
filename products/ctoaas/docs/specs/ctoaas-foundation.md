...
3. **Given** a CTO's preference data, **When** any team member of the same organization asks a question, **Then** organizational preferences are shared but individual preferences remain private to each user. [NEEDS CLARIFICATION: Should organizational preferences be shared across team members, or is this strictly single-user per organization in MVP?]
...
4. **Given** a CTO who has not input any cost data, **When** they navigate to Cloud Spend Optimization, **Then** the page shows an empty state with clear instructions on how to input spend data and an option to "Describe your infrastructure in chat" as an alternative. [NEEDS CLARIFICATION: Should Phase 1 include direct cloud provider API integration (AWS Cost Explorer, GCP Billing), or is manual input sufficient for MVP?]
...
|---|----------|---------------------|-------|--------|
| 1 | Should organizational preferences be shared across team members within the same org, or is MVP strictly single-user per organization? | Affects data model (org-level vs. user-level preference storage), onboarding flow (invite team members?), and pricing model (per-user vs. per-org). | CEO / Product Manager | Open |
| 2 | Should Phase 1 include direct cloud provider API integration for cost analysis, or is manual input sufficient for MVP? | Manual input increases friction for cost analysis. API integration adds 2-3 sprints of work. Recommendation: manual input for MVP, API integration Phase 1.5. | CEO / Architect | Open |
| 3 | What is the knowledge base content strategy for launch? How many documents/topics must be curated before MVP launch? | Insufficient knowledge base degrades RAG quality to the point where CTOaaS is no better than generic ChatGPT. BA report recommends 50 topics minimum. | Product Manager / AI-ML Engineer | Open |
| 4 | What is the pricing model for MVP launch? Free tier limits (20 messages/day assumed) and paid tier pricing ($300-2000/month range from BA report) need finalization. | Affects rate limiting implementation, feature gating, and go-to-market strategy. | CEO / Product Strategist | Open |
| 5 | Should the technology radar content be fully curated by ConnectSW, community-contributed, or AI-generated from trends data? | Curated = high quality but slow; AI-generated = fast but lower quality; community = good for engagement but moderation needed. Recommendation: curated seed (30 technologies) + AI-generated expansion with editorial review. | CEO / Product Manager | Open |
| 6 | What LLM fallback strategy should be used when Claude API is unavailable? Switch to OpenAI GPT-4, queue the request, or show a degraded experience? | Affects reliability (NFR-005) and user experience during outages. Recommendation: automatic fallback to OpenAI GPT-4 with a quality disclaimer. | Architect | Open |
