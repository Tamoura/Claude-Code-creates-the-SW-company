const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || "http://localhost:5015/api/v1";
...
/**
 * API client for CTOaaS backend.
 * Wraps fetch with auth token handling, CSRF protection for mutating
 * requests, and consistent response types.
 */
class ApiClient {
  ...
  async fetchCsrfToken(): Promise<string | null> {
    try {
      ...
      if (response.ok) {
        ...
        this.csrfToken =
          data.token ?? getCookie("_csrf") ?? null;
        return this.csrfToken;
      }
    } catch {
      // Degrade gracefully if CSRF fetch fails
    }
    return null;
  }
