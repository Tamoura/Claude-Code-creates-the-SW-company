...
export const orchestratorModeStore = atom<boolean>(initialSettings.orchestratorMode);
...
export const updateOrchestratorMode = (enabled: boolean) => {
  orchestratorModeStore.set(enabled);
  localStorage.setItem(SETTINGS_KEYS.ORCHESTRATOR_MODE, JSON.stringify(enabled));
};
