...
.claude/scripts/worktree.sh create taskflow

# Work in it
cd "$(git rev-parse --show-toplevel)/.tree/taskflow"
cd products/taskflow && npm install

# When done, remove the worktree (branch is kept)
.claude/scripts/worktree.sh remove taskflow
```
...
Claude Code creates the SW company/
  .tree/
    taskflow/     # worktree on its own branch
