...
if [ -z "$ACTION" ] || [ -z "$ACTOR" ] || [ -z "$TARGET" ]; then
  ...
  echo ""
  echo "Examples:"
  echo "  $0 TASK_COMPLETED backend-engineer archstudio/BACKEND-01 \"Implemented auth API\" \"US-01\" \"FR-001,FR-002,FR-003\""
  echo "  $0 TASK_STARTED frontend-engineer archstudio/FRONTEND-01 \"Starting canvas implementation\" \"US-04\" \"FR-007\""
  exit 1
fi
