...
SOURCE="${CONNECTSW_SOURCE:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
TARGET="$(pwd)"
...
# Control Tower — core company dashboard (not a product, part of infrastructure)
if [[ -d "$SOURCE/products/control-tower" ]]; then
  mkdir -p "$TARGET/products/control-tower"
  rsync $RSYNC_FLAGS \
    ...
    "$SOURCE/products/control-tower/" "$TARGET/products/control-tower/"
  echo "  ✓ products/control-tower/ (company dashboard)"
fi
...
|------|---------|--------|-----|
| 3113 | control-tower | Active | http://localhost:3113 |

### Backend APIs (5000-5099)

| Port | Product | Status | URL |
|------|---------|--------|-----|
| 5009 | control-tower | Active | http://localhost:5009 |
...
if [[ "$MODE" == "update" ]]; then
  ...
  echo "  • All products in products/ (except control-tower)"
  echo "  • notes/"
  echo ""
  echo "Backup:"
  echo "  • ${BACKUP_DIR:-}"
  ...
  echo "  • GitNexus skills (.claude/skills/)"
  echo "  • Control Tower dashboard (products/control-tower/)"
  ...
  echo "  2. Install Control Tower deps:"
  echo "     cd products/control-tower && npm install"
  echo "  3. Start the dashboard:"
  echo "     cd products/control-tower && npm run dev"
  echo "     → Web: http://localhost:3113  API: http://localhost:5009"
  echo "  4. Commit the setup: git add .claude .specify .githooks .github products/control-tower docs packages"
