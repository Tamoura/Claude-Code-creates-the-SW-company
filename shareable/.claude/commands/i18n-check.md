...
```
/i18n-check taskflow
/i18n-check talentgraph
/i18n-check riskdesk
```

## Arguments

- **product-name**: Product name. Works in both monorepo and single-repo layouts. All ConnectSW products targeting the the target market/MENA market require bilingual support.

## What This Command Does

This command audits a product for Arabic/English bilingual completeness. ConnectSW products serve the the target market and MENA market, where bilingual (Arabic + English) support is a regulatory and user expectation, not a nice-to-have. This check verifies:
...
# Check for currency formatting (QAR - regional Riyal)
echo "--- regional Riyal (QAR) currency formatting ---"
grep -rn --include="*.ts" --include="*.tsx" \
  -E "(QAR|regional Riyal|currency.*QAR|\u0631\.\u0642)" \
