...
# Run from Control Tower
# Navigate to /regression → select product → "Run Regression"
```

## Playwright Config

All products use always-on capture for Control Tower visibility:

```typescript
use: {
  screenshot: 'on',     // Always capture — viewable in Control Tower E2E Gallery
  video: 'on',          // Always record — viewable in Control Tower E2E Gallery
