...
```json
{
  "learned_patterns": [
    {
      "id": "TRAJ-SG-001",
      "source": "taskflow back-translation",
      ...
      "times_applied": 3,
      "success_rate": 1.0,
      "code_reference": "products/taskflow/apps/api/src/plugins/auth.ts"
    }
  ]
}
```
  ...
  "patterns": [
    {
      ...
      "source": "back-translation of taskflow, invoiceforge, pulse",
      ...
      "confidence": "high",
      "learned_from": ["taskflow", "invoiceforge", "pulse"]
    }
  ]
}
```
...
|---------|----------|-----------|
| taskflow | HIGH | Most mature, production-hardened, full-stack with SDK |
| invoiceforge | HIGH | Production with AI integration and Stripe billing |
| pulse | MEDIUM | Full-stack + mobile, complex GitHub OAuth |
| quantum-explorer | LOW | Frontend-only prototype |
| contentiq | SKIP | Still in inception, no code to back-translate |
