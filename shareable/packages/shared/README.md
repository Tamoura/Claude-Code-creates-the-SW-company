...
|--------|-------------|--------|
| `prismaPlugin` | PrismaClient lifecycle with pool sizing and graceful shutdown | taskflow |
| `redisPlugin` | Redis connection with TLS, retry, graceful degradation | taskflow |

### Utilities

| Utility | Description | Source |
|---------|-------------|--------|
| `logger` | Structured JSON logging with PII redaction | taskflow |
| `crypto` | Password hashing, API key HMAC, webhook signatures | taskflow |
